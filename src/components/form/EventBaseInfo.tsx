...
function EventBaseInfo({ value, onChange, onPeriodTimeChange }: EventBaseInfoProps) {
  ...
  return (
    <FormBox>
      <FormBox.Row label="행사 사진">
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={value.photoFiles.length === 0 && value.fileIdList.length === 0}
            className="h-11 rounded-sm border-[#cfcfcf] bg-[#efefef] text-[#666666] hover:bg-[#e2e2e2]"
          >
            삭제
          </Button>
      ...
      <FormBox.Row label="행사 일시" required contentClassName="items-start py-3">
        <BaseInfoEventDateField
          ...
          onPeriodStartDateChange={(periodStartDate) => onChange({ periodStartDate })}
          onPeriodEndDateChange={(periodEndDate) => onChange({ periodEndDate })}
          onPeriodTimeChange={onPeriodTimeChange}
          onApplyTimeToAllChange={(applyTimeToAll) => onChange({ applyTimeToAll })}
        />
      </FormBox.Row>
      ...
      <FormBox.Row label="참여 인원" required>
        <ParticipantCountRadioGroup
          ...
          onChange={(expectedParticipants) => onChange({ expectedParticipants })}
